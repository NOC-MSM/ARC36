...
CONTAINS
   ...
   SUBROUTINE stp_ctl( kt, Kmm )
      ...
      IF ( ln_SEOS.AND.(rn_b0==0._wp) ) THEN             ! Discard checks on salinity
         ...
         zminsal = -HUGE(1._wp)
      ELSE
!Stef 30 Aug 2024 increase max sal to prevent stop, high sal at seabed problem
!         zmaxsal = 100._wp
         zmaxsal = 1000._wp
         zminsal =   0._wp
      ENDIF 
...
!Stef 11 04 2025 increase velocities, boundary condition problem
...
!         & zmax(3) >   10._wp .OR.                  &         ! too large velocity ( > 10 m/s)
         & zmax(3) >   100._wp .OR.                  &         ! too large velocity ( > 10 m/s)
